...
test('approved workflow amplitude, neutral settle, progress and final price feedback remain visible',()=>{
 ...
 assert.match(css,/booking-step-out 220ms ease-out both/)
 assert.match(css,/booking-step-in 320ms cubic-bezier\(0\.2, 0\.65, 0\.3, 1\)/)
 assert.match(css,/opacity: 0\.72; transform: translate\(calc\(var\(--booking-old-x, 0px\) - 28px/)
 assert.match(css,/opacity: 0; transform: translate\(calc\(36px \* var\(--booking-direction, 1\)\), var\(--booking-enter-y, 0px\)\)/)
 assert.match(css,/--booking-direction: -1/)
 assert.match(css,/--booking-direction: 0;\s*--booking-enter-y: 12px;\s*animation-duration: 260ms/)
 assert.match(css,/transition: transform 300ms ease-out/)
 assert.match(css,/opacity: 0\.7; transform: translateY\(6px\)/)
 assert.match(css,/booking-price-state 180ms ease-out/)
